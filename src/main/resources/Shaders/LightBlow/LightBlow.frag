    ...
    if(alpha < m_AlphaDiscardThreshold){
       ...
       // IBL - Image Based Lighting. The lighting based on either cube map or sphere map.
           ...
           vec4 tempVec1;
              ...
              vec3 iblLight = texture2D(m_IblMap_Simple, vec2((((refVec) + mat * normal) * vec3(0.49)) + vec3(0.49))).rgb; // jayfella : added .rgb
           #elif  defined (IBL_SIMPLE) && !defined (NORMALMAP)
              vec3 iblLight = texture2D(m_IblMap_Simple,  vec2((refVec * vec3(0.49)) + vec3(0.49))).rgb; // jayfella : added .rgb
           #endif
...
// light.x = max(vec3(light.x), refColor);
    light.x = max(light.x, refColor.x); // jayfella : i'm not certain what the dev was trying to achieve with the line above.
